...
def alignlinearD(molecule_dictionary):
    ...
    # Calculate induced potentials V00 and V02
    V00[1 - 1, :] = - 0.25 * alphaper * E1 ** 2  # V00 for the first pulse
    V02[1 - 1, :] = - 0.25 * E1 ** 2 * (alphapar - alphaper)  # V02 for the first pulse
    V00[2 - 1, :] = - 0.25 * alphaper * E2 ** 2  # V00 for the second pulse
    V02[2 - 1, :] = - 0.25 * E2 ** 2 * (alphapar - alphaper)  # V02 for the second pulse
    V00[3 - 1, :] = - 0.25 * alphaper * E3 ** 2  # V00 for the third pulse
    V02[3 - 1, :] = - 0.25 * E3 ** 2 * (alphapar - alphaper)  # V02 for the third pulse
        ...
        if renj != 0:
            for e in earr:
                ...
                if (m[e - 1] == 0):
                    ...
                    a2, a4, a6, a8 = alignlinearmultipulses(Jmax, a - 1, 0, rotcon, centrifugal, V00, V02, t, tfree,
                    ...
                    b2, b4, b6, b8 = alignlinearmultipulses(Jmax, a - 1, m[e - 1], rotcon, centrifugal, V00, V02, t,
...
cos2thetaaverage, D2, D4, D6, D8, time, Eout, symm, mol_name = alignlinearD(molecule_dictionary)

# Plot <cos^2 theta> vs time
plt.figure(figsize=(10, 6))  # Set figure size
plt.plot(time, cos2thetaaverage, label='<cos^2 theta>', color='blue')  # Plot the curve
plt.xlabel('Time (ps)')  # Set x-axis label
plt.ylabel('<cos^2 theta>')  # Set y-axis label
plt.title('Alignment of Molecules under Laser Pulses')  # Set title
plt.legend()  # Show legend
plt.grid(True)  # Show grid

# Save the image as a PNG file
plt.savefig(f'../result/{mol_name}_cos2theta_vs_time.png', dpi=300, bbox_inches='tight')